#!/bin/bash
# Drive Vivado to synthesise and build the Arcade_Pooyan Basys3 bitstream.
#
# Usage: make_arcade_pooyan_basys3_bitstream.sh {synth|bitstream}
#
...
#   basys3/arcade_pooyan_basys3.runs/impl_1/arcade_pooyan_basys3.bit
#
...
XPR="$ROOT/basys3/arcade_pooyan_basys3.xpr"
...
WORK=/tmp/arcade_pooyan_bitstream
mkdir -p "$WORK"
...
if [ "$MODE" = "synth" ]; then
    ...
    BIT="$ROOT/basys3/arcade_pooyan_basys3.runs/impl_1/arcade_pooyan_basys3.bit"
    if [ -f "$BIT" ]; then
        echo "Bitstream: $BIT"
    else
